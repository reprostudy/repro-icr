...
from mcr.distributions.multivariate import (
    BivariateBernoulli,
    BivariateInvertible,
    BivariateSigmoidal,
    MultivariateIndependent,
)
from mcr.distributions.utils import add_uncertainty, numpyrodist_to_pyrodist
...
class GenericSCM(StructuralCausalModel):
    ...
    def __deepcopy__(self, memo):
        """
        Creates an optimized copy of the SCM.
        """
        new_scm = GenericSCM(
            dag=self.dag, costs=self.costs.copy(), y_name=self.y_name, u_prefix=self.u_prefix
        )

        # Deep copy of model dictionary
        new_scm.model = {key: val.copy() for key, val in self.model.items()}

        # Deep copy specific attributes in the model dictionary
        for key, attributes in self.model.items():
            if "values" in attributes and isinstance(attributes["values"], torch.Tensor):
                new_scm.model[key]["values"] = attributes["values"].clone()
            if "noise_values" in attributes and isinstance(
                attributes["noise_values"], torch.Tensor
            ):
                new_scm.model[key]["noise_values"] = attributes["noise_values"].clone()

        # Copy other class attributes
        new_scm.fnc_dict = self.fnc_dict.copy()
        new_scm.noise_dict = self.noise_dict.copy()
        new_scm.fnc_torch_dict = self.fnc_torch_dict.copy()
        new_scm.bounds = self.bounds.copy()

        # Assuming topological_order is a list
        new_scm.topological_order = list(self.topological_order)
    ...
    def _abduct_node_par_mcmc(
        self, node, obs, warmup_steps=200, nr_samples=400, nr_chains=1, **kwargs
    ):
        ...
        mcmc_res = GenericSCM._mcmc(
            nr_samples, warmup_steps, nr_chains, model, x_pa, x_j=x_j, rng_key=jrandom.PRNGKey(0)
        )
    ...
    def _abduct_node_obs_discrete_pyro(self, node, obs, nr_samples=1000, temperature=1):
        ...
        x_pa = torch.tensor(
            obs_df[list(self.model[node]["parents"])].to_numpy(), dtype=torch.float64
        )
        x_ch_dict = {}
        x_ch_pa_dict = {}
        for ch in self.model[node]["children"]:
            x_ch_dict[ch] = torch.tensor(obs_df[[ch]].to_numpy().flatten(), dtype=torch.float64)
            x_ch_pa_dict[ch] = torch.tensor(
                obs_df_dummy[list(self.model[ch]["parents"])].to_numpy(), dtype=torch.float64
            )
        ...
        def model_binary(x_pa, x_ch_dict=None, x_ch_pa_dict=None):
            ...
            x_j = pyro.sample(
                node, pyro.distributions.Bernoulli(probs=input), infer={"enumerate": "parallel"}
            )
            x_chs = []
            ...
            for ch_ix in range(len(chs)):
                ...
                x_ch = pyro.sample(
                    "x_{}".format(ch), d_ch, obs=x_ch_dict[ch].repeat(x_ch_input.shape)
                )
                x_chs.append(x_ch)
            return x_j, x_chs

        xj_posterior_model = pyro.infer.infer_discrete(
            model_binary, first_available_dim=-1, temperature=temperature
        )
        samples = []
        for jj in range(nr_samples):
            samples.append(
                xj_posterior_model(x_pa, x_ch_dict=x_ch_dict, x_ch_pa_dict=x_ch_pa_dict)[0].numpy()
            )

        prob = np.mean(samples)
        return dist.Binomial(probs=prob)

    def _abduct_node_obs_mcmc(
        self, node, obs, warmup_steps=1000, nr_samples=1000, nr_chains=1, **kwargs
    ):
        ...
        for ch in self.model[node]["children"]:
            ...
            if (
                not self.model[ch]["fnc"].is_invertible()
                and not self.model[ch]["fnc"].is_transformable()
            ):
                chs_infer.append(ch)
        ...
        def model_binary(x_j_obs, x_ch_dict=None, x_ch_pa_dict=None):
            ...
            for ch in chs_infer:
                ...
                x_ch = numpyro.sample(
                    "x_{}".format(ch),
                    dist.Normal(x_ch_input, GenericSCM.SMALL_VAR),
                    obs=x_ch_dict[ch],
                )
    ...
    def predict_log_prob_individualized_obs(self, obs_pre, obs_post, intv_dict, y_name, y=1):
        ...
        p_y_pre = self.model[y_name]["fnc"].raw(
            jnp.array(obs_pre[list(self.model[y_name]["parents"])])
        )
        p_y_post = scm_post.model[y_name]["fnc"].raw(
            jnp.array(obs_post[list(scm_post.model[y_name]["parents"])])
        )
        ...
        for ch in self.model[y_name]["children"]:
            ...
            if self.model[ch]["fnc"].is_invertible():
                d_ch = BivariateInvertible(
                    d_u_ch, (fnc_pre, fnc_post), (x_ch_pa_pre, x_ch_pa_post), (y_ix_pre, y_ix_post)
                )
            elif self.model[ch]["sigmoidal"]:
                d_ch = BivariateSigmoidal(
                    d_u_ch, (fnc_pre, fnc_post), (x_ch_pa_pre, x_ch_pa_post), (y_ix_pre, y_ix_post)
                )
            else:
                raise NotImplementedError(
                    "only invertible structural equation or sigmoidal supported so far."
                )
            d_chs[ch] = d_ch
    ...
    def predict_log_prob_individualized_obs_pyro(
        self, obs_pre, obs_post, intv_dict, y_name, y=1, nr_samples=1000, temperature=1
    ):
        ...
        for ch in self.model[y_name]["children"]:
            ...
            x_ch_pa_dict_pre[ch] = torch.tensor(
                obs_df_dummy_pre[list(self.model[ch]["parents"])].to_numpy()
            )

        for ch in scm_post.model[y_name]["children"]:
            x_ch_dict_post[ch] = torch.tensor(obs_df_post[[ch]].to_numpy().flatten())
            x_ch_pa_dict_post[ch] = torch.tensor(
                obs_df_dummy_post[list(scm_post.model[ch]["parents"])].to_numpy()
            )
        ...
        def model_binary(
            ...
            x_pa_post,
            x_ch_dict_pre=None,
            x_ch_pa_dict_pre=None,
            x_ch_dict_post=None,
            x_ch_pa_dict_post=None,
        ):
            assert not (
                x_pa_pre is None
                or x_pa_post is None
                or x_ch_pa_dict_pre is None
                or x_ch_pa_dict_post is None
            )
            ...
            y_pre = pyro.sample(
                y_name + "_pre",
                pyro.distributions.Bernoulli(probs=y_p_pre),
            ...
            y_post = pyro.sample(
                y_name + "_post",
                pyro.distributions.Bernoulli(probs=y_p_post),
                infer={"enumerate": "parallel"},
            )
            ...
            for ch in self.model[y_name]["children"]:
                ...
                x_ch = pyro.sample(
                    "x_{}_pre".format(ch),
                    pyro.distributions.Normal(x_ch_input, GenericSCM.SMALL_VAR**2),
        ...
        posterior_model = pyro.infer.infer_discrete(
            model_binary, first_available_dim=-1, temperature=temperature
        )
        samples = []
        for jj in range(nr_samples):
            samples.append(
                posterior_model(
                    ...
                    x_ch_pa_dict_post=x_ch_pa_dict_post,
                )[1].numpy()
            )
