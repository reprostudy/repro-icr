...
class StructuralCausalModel:
    ...
    def __init__(self, dag: DirectedAcyclicGraph, costs=None, y_name=None, u_prefix="u_"):
        ...
        # the model dictionary is an internal representation of the SCM. The keys depend
        ...
        for node in nx.algorithms.dag.topological_sort(dag.DAG):
            ...
            node, parents, children = (
                dag.var_names[node],
                [dag.var_names[node] for node in parents],
            ...
            self.model[node] = {
                ...
                "noise_distribution": (
                    None
                ),  # distribution for the noise, may be torch.tensor if point-mass probability
                "intervened": (
                    False
                ),  # indicates whether a variables was intervened upon and must therefore
                #   be ignore during reconstruction
    ...
    def __deepcopy__(self, memo):
        ...
        new_scm = StructuralCausalModel(
            self.dag, self.costs.copy(), self.predict_target, self.u_prefix
        )
        ...
        for key, attributes in self.model.items():
            ...
            if "values" in attributes and isinstance(attributes["values"], torch.Tensor):
                new_attributes["values"] = attributes["values"].clone()
            if "noise_values" in attributes and isinstance(
                attributes["noise_values"], torch.Tensor
            ):
                new_attributes["noise_values"] = attributes["noise_values"].clone()
            if "noise_distribution" in attributes and isinstance(
                attributes["noise_distribution"], torch.distributions.Distribution
            ):
                new_attributes["noise_distribution"] = attributes["noise_distribution"]

            new_scm.model[key] = new_attributes

        new_scm.topological_order = list(self.topological_order)  # Assuming it's a list
    ...
    def get_noise_values(self):
        ...
        """
        arr = torch.stack(
            [self.model[node]["noise_values"] for node in self.dag.var_names], dim=1
        ).numpy()
        df = pd.DataFrame(
            arr, columns=[self.name_to_uname(var_name) for var_name in self.dag.var_names]
        )
        return df
    ...
    def get_noise_distributions(self, check_deterministic=False, return_intervened=False):
        """
        If check deterministic we check whether all noise distributions are
            torch.tensor values (static)
        """
        noises = {}
        deterministic = True
        for node in self.dag.var_names:
    ...
    def sample_context(self, size: int, seed=None):
        ...
        Either a torch.Distribution object, a torch.tensor for point-mass distributions
        ...
        for node in self.topological_order:
            ...
            if d is None:
                ...
                self.model[node]["noise_values"] = self.model[node]["noise_distribution"].repeat(
                    size
                )
                ...
                raise NotImplementedError(
                    "The noise is neither a torch.distributions.Distribution nor torch.Tensor"
                )
        return self.get_noise_values()
    ...
    def fix_nondescendants(self, intervention_dict, obs):
        """Fix the values of nondescendants of the intervened upon-variables that
            have been observed
    ...
    def abduct_node(self, node, obs, scm_partially_abducted=None, infer_type="mcmc", **kwargs):
        ...
        # check whether node was observed, then only parents necessary for
        ...
        # NODE NOT OBSERVED AND NOT INVERTIBLE
            ...
            raise NotImplementedError(
                "No solution for variable observed but not invertible developed yet."
            )
    ...
    def abduct(self, obs, **kwargs):
        ...
        for node in self.topological_order:
            scm_abd.model[node]["noise_distribution"] = self.abduct_node(
                node, obs, scm_partially_abducted=scm_abd, **kwargs
            )
    ...
    def compute(self, do={}):
        """
        Returns a sample from SEM (observational distribution), columns are ordered as
            var self.dag.var_names in DAG
