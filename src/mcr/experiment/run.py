...
def run_experiment(
    ...
    **kwargs_model,
    ...
    t_options = ["acceptance", "improvement", "counterfactual"]
    ...
    while existing_runs < iterations:
        ...
        logging.info(
            "Creating {} batches of data with {} observations".format(N_BATCHES, batch_size)
        )
        ...
        # Turning off parallelization for the model
        ...
        if model_type == "logreg":
            ...
            if "max_depth" not in kwargs_model:
                kwargs_model["max_depth"] = 3
            if "n_estimators" not in kwargs_model:
                kwargs_model["n_estimators"] = 5
        ...
        logging.info(
            "Fitting {} models for multiplicity robustness assessment.".format(nr_refits_batch0)
        )
        model_refits_batch0 = []
