...
class GreedyEvaluator:
    def __init__(
        ...
        self._scm, self._obs, self._costs, self._features, self._lbd = (
            scm,
            obs,
            costs,
            features,
    ...
    def _evaluate(self, eta, thresh, r_type, individual):
        self.memoize_count -= 1
        intv_dict = indvd_to_intrv(
            self._scm, self._features, individual, self._obs, causes_of=None
        )
    ...
    def _evaluate_meaningful(self, gamma, r_type, individual):
        ...
        if r_type == "subpopulation":
            ...
            if len(intv_dict_causes.keys()) != len(intv_dict.keys()):
                logger.debug(
                    "Intervention dict contained interventions on non-ascendants of Y ({})".format(
                        self._y_name
                    )
                )
        ...
        if r_type == "subpopulation" and len(intv_dict_causes.keys()) == 0:
            ...
            perc_positive = torch.exp(
                self._scm.predict_log_prob_obs(self._obs, self._y_name, y=1)
            ).item()
        else:
            # sample from intervened distribution for obs_sub
            values = scm_.compute(do=intv_dict)
