...
def recourse_population(
    ...
    logging.debug(
        "Determining rejected individuals and individuals determined to implement recourse..."
    )
    predictions = np.exp(predict_log_proba(X)[:, 1]).flatten() >= thresh
    ixs_rejected = np.arange(len(predictions))[predictions == 0]
    ixs_recourse = np.random.choice(
        ixs_rejected, size=math.floor(proportion * len(ixs_rejected)), replace=False
    )
    ...
    logging.debug(
        "Detected {} rejected and {} recourse seeking individuals...".format(
            len(ixs_rejected), len(ixs_recourse)
        )
    )
    ...
    if t_type == "improvement":
        ...
        causes = [
            nd for nd in intv_features if nd in causes_dag
        ]  # to make sure the ordering is as desired
        ixs_causes = np.array(
            [np.arange(len(intv_features))[cause == intv_features][0] for cause in causes]
        )
    ...
    logging.debug(
        "Iterating through {} individuals to suggest recourse...".format(len(ixs_recourse))
    )

    ixs_recourse_it = tqdm(ixs_recourse) if verbose else ixs_recourse

    for ix in ixs_recourse_it:
        obs = X.iloc[ix, :]
        ...
        if r_type == "subpopulation" or t_type == "counterfactual":
            ...
            raise NotImplementedError(
                "r_type must be in {}".format(["individualized", "subpopulation"])
            )

        _ = scm_.sample_context(size=nsamples)
        winner, goal_cost, intv_cost = recourse(
            scm_,
            intv_features,
            obs,
            costs,
    ...
    ixs_rp = interventions[
        np.abs(interventions.abs().sum(axis=1)) > 0
    ].index  # indexes where recourse was performed
    ...
    if r_type == "individualized" and t_type == "improvement" and predict_individualized:
        ...
        h_post_indiv = compute_h_post_individualized(
            scm, X_pre, X_post, interventions, intv_features, y_name, y=1
        )
