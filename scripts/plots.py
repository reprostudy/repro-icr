...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--savepath",
        help="path to savepath",
        type=str,
        default="../experiments/remote-experiments/neurips/",
    )
    ...
    def compile_df_plt(dfss, value_types, value_names):
        ...
        for value in value_types:
            ...
            if value == "eta_obs_individualized":
                ixs_indMCR = np.logical_and(
                    df_plt["method"] == "ICR", df_plt["type"] == "individualized"
                )
    ...
    for idx, name in enumerate(metric_sets.keys()):
        df_plt = compile_df_plt(dfss, metric_sets[name], value_names)

        with sns.plotting_context("talk"):
            legend = True if name == "acceptance" else False
            ...
            for ii in range(len(g2.axes.flat)):
                ...
                if "cost" not in title:
                    ax.set_yscale(
                        "function", functions=[lambda x: x**2, lambda z: np.power(z, 1 / 2)]
                    )
                    ax.set_ylim(*ylim)
                    ax.set_yticks(ticks)
    ...
    table_all = result_table.merge(
        result_table_std, left_index=True, right_index=True, suffixes=("_mean", "_std")
    )
    column_order = [
        "gamma_obs_mean",
        "gamma_obs_std",
        "eta_obs_mean",
        "eta_obs_std",
        "eta_obs_individualized_mean",
        "eta_obs_individualized_std",
